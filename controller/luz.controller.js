...
const debug = require("debug")("app:luz-controller");
  ...
  try {
    ...
    const savedData = await Promise.all(
      dataArray.map(async ({ key, value }) => {
        ...
        const luz = new Luz1({
          Luz: value, // Use the value from the array
        });

        return await luz.save();
      })
    );
...
controller.getLuz = async (req,res) =>{
    try {
    const data = await Luz1.find().lean(); // Retrieve all documents
